...
$(document).ready(function() {
	...
	$("#btn_confirm_matrix").click(function() {
		...
		masCoreGraph.length = 0;
		masMissingVertex2.length = 0;
		...
		// I. ВНУТРЕННЯЯ УСТОЙЧИВОСТЬ
		
			$("#resultMatrixContiguity").append("<p>I. ВНУТРЕННЯЯ УСТОЙЧИВОСТЬ</p>");
			...
			// Нахождение максимальной внутренней устойчивости
				...
				$("#internalStability").append("<li>Число внутренней устойчивости (максимальное число подмножеств): " + maxValue + "</li>");
			
			// Конец нахождения максимальной внутренней устойчивости
			
		// КОНЕЦ ВНУТРЕННЕЙ УСТОЙЧИВОСТИ
		
		
		// II. ВНЕШНЯЯ УСТОЙЧИВОСТЬ
		
			$("#resultMatrixContiguity").append("<p>II. ВНЕШНЯЯ УСТОЙЧИВОСТЬ</p>");
		
			$("#resultMatrixContiguity").append('<ol id="outerStability"></ol>');
		
			// Заполнение главной диагонали матрицы смежности единицами	
			...
			// Конец заполнения главной диагонали матрицы смежности единицами
		
			
			// Вывод матрицы смежности
				
				$("#outerStability").append("<li>Матрица смежности:</li>");
				
				$("#outerStability").append('<table id="matrixContiguity2"></table>');
			...
			// Конец вывода матрицы смежности
			
			
			// Нахождение строчных дизъюнктов  
				
				$("#outerStability").append("<br />");
				$("#outerStability").append("<li>Строчные дизъюнкты:</li>");
				...
				for (var i = 0; i < countVertex; i++) {
		    	  ...
		    	  for (var j = 0; j < countVertex; j++) {
		    		  if (matrixContiguity[i][j] == 1) {
							...
							// Если вершины - буквы
							if (typeVertex == 2) {
								mas[mas.length] = masForLetterVertex[j];							
							}		
				  ...
				  for (var j = 0; j < mas.length; j++) {
					if (j != mas.length-1) {
						disjunctiveCouple += mas[j] + "v";
					}
					else {
						disjunctiveCouple += mas[j];
					}
				  }
				...
				// Вывод дизъюнктивнфх пар
				$("#outerStability").append("<p> КНФ: " + disjunctiveCouple + "</p>");
				
			// Конец нахождения строчных дизъюнктов  
			
			
			// Нахождение ДНФ у дизъюнктивных пар
			
				$("#outerStability").append('<li>ДНФ у дизъюнктивных пар:</li>');
			
				// Вызываем свою пользовательскую функцию
				cnfToDnf(disjunctiveCouple, 2);
			
			// Конец нахождения ДНФ у дизъюнктивных пар
			
			
			// Выписываем вершины из каждой элементарной конъюнкции
			
				for (var i = 0; i < masForOperandResult.length; i++) {  
					masMissingVertex2[masMissingVertex2.length] = masForOperandResult[i];
				}
			
			// Конец выписывания вершин из каждой элементарной конъюнкции
			
			
			// Вывод массива, который содержит недостающие вершины 
			
				$("#outerStability").append('<li>Вершины у каждой элементарной коньюнкции:</li>');
			
				$("#outerStability").append("<br />");
		
				for (var i = 0; i < masMissingVertex2.length; i++) {
					
					$("#outerStability").append("{");
					
					for (var j = 0; j < masMissingVertex2[i].length; j++) {
						
						if (j == masMissingVertex2[i].length-1) {
							$("#outerStability").append(masMissingVertex2[i].charAt(j));
						}
						else {
							$("#outerStability").append(masMissingVertex2[i].charAt(j) + ", ");
						}
			...
			// Нахождение максимальной внутренней устойчивости
				...
				for (var i = 0; i < masMissingVertex2.length; i++) {
					if (i == 0) {
						minValue = masMissingVertex2[i].length;
					}
					else {
						if (minValue > masMissingVertex2[i].length) {
							minValue = masMissingVertex2[i].length;
						}
					}  
				}
				
				$("#outerStability").append('<li>Число внешней устойчивости (минимальное число подмножеств): ' + minValue + '</li>');
			
			// Конец нахождения максимальной внутренней устойчивости
			
		// КОНЕЦ ВНЕШНЕЙ УСТОЙЧИВОСТИ
		
		
		// ПОИСК ЯДРА ГРАФА	  
		
			$("#resultMatrixContiguity").append("<p>Множества, одновременно внутренне и внешне устойчивые называются ядром графа: </p>");
		
			countCoreGraph = 0;
	   	    
	   	    for (var i = 0; i < masMissingVertex.length; i++) {
	   	    	for (var j = 0; j < masMissingVertex2.length; j++) {
					if (masMissingVertex[i] == masMissingVertex2[j]) {
						masCoreGraph[masCoreGraph.length] = masMissingVertex[i];
		   	    	}
		   	    }
	   	    }
			...
			// Вывод ядер графа
				...
				$("#resultMatrixContiguity").append("<p>Количество ядер графа: " + masCoreGraph.length + "</p>");
			
				if (masCoreGraph.length != 0) {
	   	    		
					$("#resultMatrixContiguity").append("<p>Ядра: </p>");

	   	    		for (var i = 0; i < masCoreGraph.length; i++) {
	   	    			
	   	    			$("#resultMatrixContiguity").append("{");
	   	    			
	   		   	    	for (var j = 0; j < masCoreGraph[i].length; j++) {
	   			   	    	if (j == masCoreGraph[i].length-1) {
								$("#resultMatrixContiguity").append(masCoreGraph[i].charAt(j));
	   			   	    	}
	   			   	    	else {
								$("#resultMatrixContiguity").append(masCoreGraph[i].charAt(j)+ ", ");
							}
			...
			// Конец вывод ядер графа
		
		// КОНЕЦ ПОИСКА ЯДРА ГРАФА
